...
setwd("E:/nede/Actuariat")

## 1. Chargement des données HMD:

danemark <- read.demogdata(file="E:/nede/Actuariat/Mx_1x1.txt", 
                           popfile="E:/nede/Actuariat/Exposures_1x1.txt",
                           type="mortality", label="Danemark")
class(danemark)
names(danemark)


# l'annee 1988 pour les femmes
head(danemark$rate$female[, "1988"])

# 6 premieres annees de la population totale
head(danemark$rate$total[, 1:6])
...
par(mfrow=c(1,1))
plot(danemark$age, log(danemark$rate$total[,"1988"]), main ='log mortality rates (Danemark, 1988)',
     xlab = "Ages x", ylab = "log mortality rates", type = "l")


## 2. Affichage des objets demogdata: 

plot(danemark, series = 'total')
plot(danemark, series = 'male')
plot(danemark, series = 'female')

## avec légendes des années  
legend("bottomright",legend=unique(danemark$year),
       col=rainbow(length(danemark$year)*1.25), ncol=5, pch=3, 
       title="Year", cex=0.2)

## avec légendes d'âges
legend("bottomright",legend=unique(danemark$age),
       col=rainbow(length(danemark$age)*1.25), ncol=5, pch=3, 
       title="Âges", cex=0.3)


#Il est possible de sélectionner des années
dnk_years = c(1945,1962,1979,1996,2013)
plot(danemark, series = 'total', years = dnk_years ,
     main = "Danemark total death rates for years (1945, 1962, 1979, 1996, 2013)"
     )
legend(x="bottomright", legend = dnk_years,
       col = rainbow(length(dnk_years)*1.25),
       lty = 1,
       cex=0.7,
       box.lwd = 0.3
       ) 
...
dnk_ages = c(0,27,55,82,110)
plot(danemark,series="male",datatype="rate", plot.type="time", 
     main="Male rates",xlab="Years")
legend(x="bottomleft", legend = dnk_ages,
       col = rainbow(length(dnk_ages)*1.25),
       lty = 1,
       cex=0.6,
       box.lwd = 0.3
) 

plot(danemark,series="female",datatype="rate", plot.type="time", main="Female rates",xlab="Years")
plot(danemark,series="total",datatype="rate", plot.type="time", main="Total rates",xlab="Years")



# il est possible de séléctionner des âges pendant une période d'années
dnk.mort_age = extract.ages(danemark,0:60,F)    
dnk.mort_age_yr = extract.years(dnk.mort_age,1918:1950)    
plot(dnk.mort_age_yr,series='male')   
legend("bottomright",legend=unique(dnk.mort_age_yr$year),
       col=rainbow(length(dnk.mort_age_yr$year)*1.25), ncol=5, pch=19, 
       title="Year", cex=0.5)
...
plot(danemark, series ='male',years=2003, type="p", pch=1 )
# (voir smoothing 2 et 3)


smus = smooth.demogdata(danemark)
plot(danemark, series ='male',years=2003, type="p", pch=1, col='gray' )
lines(smus, series = 'male', years = 2003)


##2 types de lissages:
danemark1914 <- extract.years(danemark, years=1914:2013)
danemark.smth.m <- smooth.demogdata(danemark1914, method="mspline") #spline monotone
danemark.smth.u <- smooth.demogdata(danemark1914, method="spline")  #spline standard
par(mfrow=c(1,2))

plot(danemark, years=1945, type="p", pch=21, ylim=c(-7, 1))
lines(danemark.smth.m, years=1945)
lines(danemark.smth.u, years=1945, lty=3)


library(lifecontingencies)

# Calculs actuariels d'après les taux de 2013
qx_2016 = danemark$rate$total[0:108,"2016"]
...
danemark.momo<-StMoMoData(data=danemark, series = "total",type="central")

names(danemark.momo)
head(danemark.momo$Ext[,'2013']) #???Exposition au risque de l'année 2013
...
#Fitting the model
LC <- lc(link = "logit")
LCfit <- fit(LC, data = central2initial(danemark.momo), 
                                        ages.fit = ages.fit,
                                        years.fit = years.fit)
...
LCfit2 <- fit(LC, data = central2initial(danemark.momo), 
             ages.fit = ages.fit,
             years.fit = 1835:2003)
...
rates<-cbind(danemark$rate$total[0:99,1:180],LCfor$rate[0:99,])
head(rates)

plot(seq(min(danemark$year[1:180]),max(danemark$year[1:180])+horizon),
      rates[65,],
...
#2éme contrat:
K = 100000
VAP_capdecR = K * Axn(lc_acttbl_2016, x=40,i=i)
VAP_capdecR_1976 = K * Axn(lc_acttbl_1976, x=40,i=i)
VAP_capdecR
VAP_capdecR_1976
VAP_capdec
...
#2éme contrat:
Prime_capdec1=VAP_capdecR_1976/axn(lc_acttbl_1976, x=40,i=i)
Prime_capdec1
Prime_capdec=(k*Axn(lc_acttbl_1976, x=40,i=i))/axn(lc_acttbl_1976, x=40,i=i)
Prime_capdec
